...
const rooms: { [key: string]: Room } = {
  bedroom: {
    ...
    description:
      "Your bedroom is a shrine to 80s computing. Posters of Tron and WarGames adorn the walls. Your trusty Commodore 64 sits on the desk to the SOUTH, its beige plastic gleaming. A closed wardrobe is to the NORTH. An overturned office chair with squeaky wheels lies near the desk. Your unmade bed is to the WEST. The door to freedom (and breakfast) is to the EAST. A light switch is mounted on the wall by the door.",
    darkDescription:
      "It's pitch black. You can't see a thing. You hear muffled sounds from somewhere beyond your room. Your bed is somewhere behind you. You'll need to feel around in the dark to find your bearings.",
    exits: { east: "hallway" },
    items: [],
  },
  hallway: {
...
function handleMove(direction: string): void {
  ...
  if (gameState.currentRoom === "bedroom" && direction === "west") {
    if (!gameState.lightsOn) {
      if (!gameState.fellIntoBed) {
        ...
        printLine("");
        printLine(
          "You fall backwards onto something soft. It's your bed!",
          "info"
        );
        printLine(
          "Comfortable, but you should probably get up and find the lights.",
          "info"
        );
    ...
    printLine(
      "You look at your unmade bed. Tempting, but you've got a party to get to!",
      "info"
    );
    return;
  }
  ...
  if (gameState.currentRoom === "bedroom" && direction === "north") {
    if (!gameState.lightsOn) {
      ...
      printLine("");
      printLine(
        "You bump into something large and wooden. It feels like a wardrobe or closet.",
        "info"
      );
  ...
  if (direction === "east" && gameState.currentRoom === "bedroom") {
    if (!gameState.lightsOn) {
      ...
      printLine("");
      printLine(
        "Your hand touches a solid wooden door. You feel around the frame...",
        "info"
      );
      printLine(
        "Aha! There's a light switch on the wall next to the door!",
        "success"
      );
      return;
    }
